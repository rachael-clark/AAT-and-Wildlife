...
#SEIR Model for Four Vertebrate species (three wild and one domestic)
#and G. swynnertoni tsetse species as the vector
seir4.gswyn <- function(times, init, parameters) {
...
buf.adj.gswyn <- buf.gswyn / (buf.gswyn + gir.gswyn + ele.gswyn) * wildlife.prop
gir.adj.gswyn <- gir.gswyn/(buf.gswyn + gir.gswyn + ele.gswyn) * wildlife.prop
ele.adj.gswyn <- ele.gswyn/(buf.gswyn + gir.gswyn + ele.gswyn) * wildlife.prop

ifelse(buf.adj.gswyn + gir.adj.gswyn + ele.adj.gswyn == wildlife.prop, "TRUE", "FALSE")

domestic.prop <- 1-wildlife.prop
hum.gswyn <- 0.3
cat.gswyn <- 0.7

hum.adj.gswyn <- hum.gswyn / (hum.gswyn + cat.gswyn) * domestic.prop
cat.adj.gswyn <- cat.gswyn / (hum.gswyn +cat.gswyn) * domestic.prop

ifelse(hum.adj.gswyn + cat.adj.gswyn == domestic.prop,"TRUE", "FALSE")
...
parms4.gswyn <- c(
  ...
  a1=cat.adj.gswyn/d,     #portion of tsetse bloodmeals/duration of feeding cycles in fly
  #for host 1
  ...
  a2=buf.adj.gswyn/d,      #portion of tsetse bloodmeals/duration of feeding cycles in fly
  #for host 2
  ...
  a3= gir.adj.gswyn/d,     #proportion of tsetse bloodmeals/duration of feeding cycles in fly
  #for host 3
  ...
  a4= ele.adj.gswyn/d,     #proportion of tsetse bloodmeals/duration of feeding cycles in fly
  #for host 4
...
out4.gswyn <- as.data.frame(ode(init4.gswyn, time, seir4.gswyn, parms4.gswyn))
...
# Vector
plot(out4$`SusV` ~out4.gswyn$time, type='l', col='darkgreen', 
     xlab= 'time', ylab='N', ylim = c(0,5000), lwd = 2)
lines(out4.gswyn$'ExpV'~out4.gswyn$time, col='orange', lwd = 2)
lines(out4.gswyn$'InfV'~out4.gswyn$time, col='red', lwd = 2)
...
out4.gswyn$Total1 <- out4.gswyn$Sus1 + out4.gswyn$Exp1 + out4.gswyn$Inf1 + out4.gswyn$Rec1
out4.gswyn$Total2 <- out4.gswyn$Sus2 + out4.gswyn$Exp2 + out4.gswyn$Inf2 + out4.gswyn$Rec2
out4.gswyn$Total3 <- out4.gswyn$Sus3 + out4.gswyn$Exp3 + out4.gswyn$Inf3 + out4.gswyn$Rec3
out4.gswyn$Total4 <- out4.gswyn$Sus4 + out4.gswyn$Exp4 + out4.gswyn$Inf4 + out4.gswyn$Rec4
out4.gswyn$TotalV <- out4.gswyn$SusV + out4.gswyn$ExpV + out4.gswyn$InfV
View(out4.gswyn)
